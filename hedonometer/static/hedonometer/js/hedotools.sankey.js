...
hedotools.sankey = function() { 
	...
	pathselection = axes.selectAll("path.sankey").data(sankeydata)
		...
		if (useTip) {
			...
			// var reffile = "https://hedonometer.org/data/cities/word-vectors/"+reftimeseldecoder().cached+"/"+d.name+".csv";
			// if (parseInt(reftimeseldecoder().cached) < 2014) reffile+=".new"
			// var compfile = "https://hedonometer.org/data/cities/word-vectors/"+comptimeseldecoder().cached+"/"+d.name+".csv";
