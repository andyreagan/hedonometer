...
var toggleAll = function() {
	if (legendDict['togall'] == 'on') {
		d3.selectAll(".Monday, .Tuesday, .Wednesday, .Thursday, .Friday, .Saturday, .Sunday, .Togall").transition().duration(250).attr("r", rmax);
	}
	...
	var toggleDays = function() {
			...
			if (legendDict['tue'] == 'on') d3.selectAll(".Tuesday").transition().duration(250).attr("r", rmax);
			if (legendDict['wed'] == 'on') d3.selectAll(".Wednesday").transition().duration(250).attr("r", rmax);
			if (legendDict['thu'] == 'on') d3.selectAll(".Thursday").transition().duration(250).attr("r", rmax);
			if (legendDict['fri'] == 'on') d3.selectAll(".Friday").transition().duration(250).attr("r", rmax);
			if (legendDict['sat'] == 'on') d3.selectAll(".Saturday").transition().duration(250).attr("r", rmax);
			if (legendDict['sun'] == 'on') d3.selectAll(".Sunday").transition().duration(250).attr("r", rmax);
			if (legendDict['hilite'] == 'on') d3.selectAll(".Hilite").transition().duration(250).attr("visibility", "visible");

			if (legendDict['mon'] == 'off') {
				d3.selectAll(".Monday").transition().duration(250).attr("r", rmin);
			}
			if (legendDict['tue'] == 'off') d3.selectAll(".Tuesday").transition().duration(250).attr("r", rmin);
			if (legendDict['wed'] == 'off') d3.selectAll(".Wednesday").transition().duration(250).attr("r", rmin);
			if (legendDict['thu'] == 'off') d3.selectAll(".Thursday").transition().duration(250).attr("r", rmin);
			if (legendDict['fri'] == 'off') d3.selectAll(".Friday").transition().duration(250).attr("r", rmin);
			if (legendDict['sat'] == 'off') d3.selectAll(".Saturday").transition().duration(250).attr("r", rmin);
			if (legendDict['sun'] == 'off') d3.selectAll(".Sunday").transition().duration(250).attr("r", rmin);
			if (legendDict['hilite'] == 'off') d3.selectAll(".Hilite").transition().duration(250).attr("visibility", "hidden");
		}
	...
	thelegend.append("svg:g").attr("width", 100).append("svg:circle").on("mousedown", function() {
		...
		toggleDays();
	}).attr("cx", 8).attr("cy", 15).attr("r", rmax).attr("stroke", "black").attr("stroke-width", 0.7).attr("class", "Sunday");

	thelegend.append("svg:g").append("svg:text").attr("x", 15).attr("y", 20).on("mousedown", function() {
		if (legendDict['sun'] == "on") {
			legendDict['sun'] = "off";
		} else {
			legendDict['sun'] = "on";
	...
	thelegend.append("svg:g").append("svg:text").attr("x", 56).attr("y", 20).on("mousedown", function() {
		if (legendDict['mon'] == "on") {
			legendDict['mon'] = "off";
		} else {
			legendDict['mon'] = "on";
	...
	thelegend.append("svg:g").append("svg:text").attr("x", 96).attr("y", 20).on("mousedown", function() {
		if (legendDict['tue'] == "on") {
			legendDict['tue'] = "off";
		} else {
			legendDict['tue'] = "on";
	...
	thelegend.append("svg:g").append("svg:text").attr("x", 131 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['wed'] == "on") {
			legendDict['wed'] = "off";
		} else {
			legendDict['wed'] = "on";
	...
	thelegend.append("svg:g").append("svg:circle").on("mousedown", function() {
		...
		toggleDays();
	}).attr("cx", 172).attr("cy", 15).attr("r", rmax).attr("stroke", "black").attr("stroke-width", 0.7).attr("class", "Thursday");

	thelegend.append("svg:g").append("svg:text").attr("x", 172 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['thu'] == "on") {
			legendDict['thu'] = "off";
		} else {
			legendDict['thu'] = "on";
		...
		toggleDays();
	}).text("Thu").attr("class", "Thursday");

	thelegend.append("svg:circle").on("mousedown", function() {
	...
	thelegend.append("svg:text").attr("x", 213 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['fri'] == "on") {
			legendDict['fri'] = "off";
		} else {
			legendDict['fri'] = "on";
	...
	thelegend.append("svg:text").attr("x", 251 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['sat'] == "on") {
			legendDict['sat'] = "off";
		} else {
			legendDict['sat'] = "on";
		...
		toggleDays();
	}).text("Sat").attr("class", "Saturday");
	
	
	thelegend.append("svg:circle").on("mousedown", function() {
		if (legendDict['hilite'] == "on") {
			legendDict['hilite'] = "off";
		} else {
			legendDict['hilite'] = "on";
		}
		toggleDays();
	}).attr("cx", 292).attr("cy", 15).attr("r", rmax).attr("stroke", "black").attr("stroke-width", 0.7).attr("class", "Hilite")
	
	thelegend.append("svg:text").attr("x", 292 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['hilite'] == "on") {
			legendDict['hilite'] = "off";
		} else {
			legendDict['hilite'] = "on";
		...
		toggleDays();
	}).text("Shocks").attr("class", "hilite");
	//41
	thelegend.append("svg:circle").on("mousedown", function() {
		if (legendDict['togall'] == "on") {
			legendDict['togall'] = "off";
		} else {
			legendDict['togall'] = "on";
		}
		toggleAll();
	}).attr("cx", 360).attr("cy", 15).attr("r", rmax).attr("stroke", "black").attr("stroke-width", 0.7).attr("class", "Togall")
	
	thelegend.append("svg:text").attr("x", 360 + 6).attr("y", 20).on("mousedown", function() {
		if (legendDict['togall'] == "on") {
			legendDict['togall'] = "off";
			legendDict['mon'] = "off";
			legendDict['tue'] = "off";
...
function selectYear(year) {
		...
		d3.csv("http://s3.amazonaws.com/hedopostprocess/daily" + year + ".csv", function(d) {
			function getDate(d) {
...
//		timeline(year);
//		if (year == "Full") d3.select(".infobox").append("h4").text("Daily Happiness Averages for Twitter, September 2008 to present");
//		else d3.select(".infobox").append("h4").text("Daily Happiness Averages for Twitter, " + year);
//		d3.csv("http://s3.amazonaws.com/hedopostprocess/daily" + year + ".csv", function(data) {
	...
	theyears.append("svg:rect").attr("x", 55 * 7).attr("y", 10).attr("width", 10).attr("height", 10).attr("id", "rect2014").on("mouseup", function() {
		d3.select("#rect2014").style("fill", "black");
	}).on("mousedown", function() {
		selectYear("2014")
	})

	theyears.append("svg:text").attr("x", 55 * 7 + 15).attr("y", 20).text("2014").on("mouseup", function() {
		d3.select("#rect2014").style("fill", "black");
	}).on("mousedown", function() {
		selectYear("2014")
	})
	...
	theyears.append("svg:rect").attr("x", 55 * 8).attr("y", 10).attr("width", 10).attr("height", 10).attr("id", "rectFull").on("mouseup", function() {
		d3.select("#rectFull").style("fill", "black");
	}).on("mousedown", function() {
		selectYear("Full")
	})

	theyears.append("svg:text").attr("x", 55 * 8 + 15).attr("y", 20).text("Full Range").on("mouseup", function() {
		d3.select("#rectFull").style("fill", "black");
	}).on("mousedown", function() {
		selectYear("Full")
	})
	...
	function timeline(year) {
		...
		d3.csv("http://s3.amazonaws.com/hedopostprocess/daily" + year + ".csv", function(data) {
			...
			var path = focus.append("path").attr("id", "path").data([data]).attr("clip-path", "url(#clip)").attr("d", area);
			...
			.transition().duration(dur)
			.attr("r", function(d){return rmax*(1)})
			...
			var data2 = [
			  {},
			  {date: new Date(2008,  8, 29), value:5.95, caption: 'Bailout'},
			  {date: new Date(2009,  3, 27), value:6.901, caption: 'Swine flu'},
			  {date: new Date(2009,  5, 25), value:5.91, caption: 'MJ death'},
//			  {date: new Date(2009,  7, 6), value:6.01, caption: 'ddos twitter'},
			  {date: new Date(2009,  8, 14), value:5.99, caption: 'Swayze dead'},
			  {date: new Date(2010,  1, 27), value:6.00, caption: 'Chile Quake'},
//			  {date: new Date(2010,  5, 27), value:6.01, caption: 'Germany 4-1 England'},
			  {date: new Date(2011,  2, 11), value:5.93, caption: 'Tsunami Japan'},
			  {date: new Date(2011,  3, 29), value:6.04, caption: 'Royal Wedding'},
			  //{date: new Date(2011,  6, 23), value:5.91, caption: 'Norway'},
			  {date: new Date(2011,  7, 8), value:5.94, caption: 'London Riots'},
			  {date: new Date(2012,  11, 14), value:5.89, caption: 'Newtown'},
			  {date: new Date(2013,  3, 15), value:5.89, caption: 'Boston'},

			];
			
			var rect = focus2.selectAll("rect").data(data2);
			var text = focus2.selectAll("text").data(data2);
			var format = d3.time.format("%m-%d");
			

			text.enter().append("text").text(function(d){return d.caption;}).attr("class","Hilite").attr("dx", function(d){return x(d.date)}).attr("dy", function(d){
				if(d.value <= 6.05)
					return y(d.value) -.4;
				else
					return y(d.value) - 10.4;
				;
				}
				);
/*
			rect.enter().append("rect").attr("width", rmax*10).attr("height", rmax*10).attr("class", function(d) {
				
					return getDay(d);
			}).attr("x", function(d) {
				return x(d.date);
			}).attr("clip-path", "url(#clip)").attr("y", function(d) {
				if(d.value <= 6.05)
					return y(d.value) -.4;
				else
					return y(d.value) - 20.4;
				;
			}).attr("shortdate", function(d) {
				return d.shortDate;
			}).attr("havg", function(d) {
			...
			.append("text").text(function(d){return d.caption;}).attr("dx", function(d){return x(d.date)});
*/		
			text.exit().remove();
			//rect.exit().remove();
		...
		function brush() {
			...
			var text = focus2.selectAll("text").attr("dx", function(d) {
				return x(d.date);
			}).attr("dy", function(d) {
				if(d.value <= 6.05)
					return y(d.value);
		...
		var myMouseOverFunction = function() {
				...
				d3.json("http://s3.amazonaws.com/hedopostprocess/word-shift-results/data-short/" + circle.attr("shortdate") + ".json", function(json) {
					...
					shortlist.append("svg:rect").attr("id", "shadow").attr("x", 12).attr("y", 5).attr("width", miniboxWidth + 5)
					...
					shortlist.append("svg:rect").attr("x", 10).attr("y", 0).attr("width", miniboxWidth + 5) 
