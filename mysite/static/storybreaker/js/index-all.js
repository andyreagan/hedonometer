...
function plotShift(figure,sortedMag,sortedType,sortedWords,sumTypes,refH,compH) {
    ...
    height = 450,
    boxwidth = (width-20-10),
    boxheight = (height-40-25),
    ...
    yHeight = (7+17*3+14+5-13), // 101
    clipHeight = 100-20-13,
    barHeight = (7+17*3+15-13), // 95
    innerwidth = (figwidth-20), // give just enough room for the labels
    innerheight = (figheight-20),
    bigfigcenter = innerwidth/2;

    var svg = figure.append('svg') // figure = d3.select('#moveshifthere')
	.attr('height',height).attr('width',width)
	.attr('id','shiftsvg');

    var canvas = svg.append("svg")
	.attr("id","shiftcanvas")
	.attr("width",function () { return width-20-10; })
	.attr("height",function () { return height-25; });

    // x label of shift, outside of the SVG
    svg.append("text")
	.text("Per word average happiness shift")
	.attr("class","axes-text")
	.attr("x",(width-20-10)/2+20) // 350-20-10 for svg width,  
	.attr("y",height-7)
	...
	.range([maxWidth+10,innerwidth-maxWidth-10]);
    ...
    axes.append("rect").attr("x",3).attr("y",3).attr("width",innerwidth-6).attr("height",73-13).attr("fill","white").style({"opacity": "1.0"});
	...
	.range([innerwidth*.12,innerwidth*.88]);
...
function drawTimeseries() {
    var width = parseInt(d3.select("#timeseries01").style("width")),
    height = width*1.2;

    d3.text("/storybreaker/api/timeseries/2014-07-21-00-00/2014-07-21-01-00/en/240/0", function(error, data) {
	console.log(data);
	var tmp = data.split("\n").slice(1,data.length);
	console.log(tmp);
	...
	var dataobject = dataarray.map(function(d) { return {"time": d[0], "happs": parseFloat(d[1]),}; });
	...
	d3.select("#timeseries01").selectAll("svg.canvas").remove();
	...
	var line = d3.svg.line()
	    ...
	    .y(function(d,i) { return d.happs; });

	var linegroup = axes.append("g")
