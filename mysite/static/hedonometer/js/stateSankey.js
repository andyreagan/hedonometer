// make the plot
function plotSankey(figure,olddata,newdata,geodata) {
    /* plot the bar chart
    ...
    */
    ...
    figwidth = parseInt(d3.select('#sankeyChart').style('width')) - margin.left - margin.right,
    aspectRatio = 2.2,
    figheight = parseInt(d3.select('#sankeyChart').style('width'))*aspectRatio - margin.top - margin.bottom,
    ...
    // do the sorting
    oldindices = Array(olddata.length);
    for (var i = 0; i < olddata.length; i++) { oldindices[i] = i; }
    ...
    oldindices.sort(function(a,b) { return olddata[a] < olddata[b] ? 1 : olddata[a] > olddata[b] ? -1 : 0; });


    // do the sorting on new data
    newindices = Array(newdata.length);
    for (var i = 0; i < newdata.length; i++) { newindices[i] = i; }
    newindices.sort(function(a,b) { return newdata[a] < newdata[b] ? 1 : newdata[a] > newdata[b] ? -1 : 0; });
    var sortedStatesNew = Array(newdata.length);
    for (var i = 0; i < newdata.length; i++) { sortedStatesNew[i] = [i,newindices[i],geodata[newindices[i]].properties.name,newdata[newindices[i]]]; }
    console.log(sortedStatesNew);
    
    // now build the old list, with an additional entry
    sortedStatesOld = Array(olddata.length);
    for (var i = 0; i < olddata.length; i++) { sortedStatesOld[i] = [i,oldindices[i],geodata[oldindices[i]].properties.name,olddata[oldindices[i]],Math.abs(olddata[oldindices[i]]-newdata[oldindices[i]]),newindices.indexOf(oldindices[i])]; }
